...
class HybridRAMBOPolicy(MOPOPolicy):
    """
    ...
    def update_dynamics(
        ...
        while steps < self._adv_train_steps:
            ...
            for t in range(self._adv_rollout_length):
                ...
                offline_observations, offline_actions, offline_next_observations, offline_rewards = \
                    itemgetter("observations", "actions", "next_observations", "rewards")(real_buffer.sample(offline_buffer_size))
                ...
                online_rewards = torch.tensor(online_buffer['rewards'], device=offline_observations.device)
                ...
                sl_rewards = torch.cat((online_rewards, offline_rewards), dim=0)
