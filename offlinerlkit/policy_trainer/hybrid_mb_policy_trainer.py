...
# hybrid model-based policy trainer
class HybridMBPolicyTrainer:
    def __init__(
        self,
        policy: BasePolicy,
        eval_env: gym.Env,
        ...
        for e in range(1, self._epoch + 1):
            ...
            for it in pbar:
                ...
                if 0 < self._dynamics_update_freq and (num_timesteps+1)%self._dynamics_update_freq == 0:
                    ...
                    dynamics_update_info = self.policy.update_dynamics(self.real_buffer, online_buffer)
    ...
    def _get_online_buffer(self, online_buffer_size) -> Dict[str, torch.tensor]:
        ...
        buffer['observations'] = np.vstack(buffer['observations'])
        buffer['actions'] = np.vstack(buffer['actions'])
        buffer['next_observations'] = np.vstack(buffer['next_observations'])
        buffer['rewards'] = np.vstack(buffer['rewards'])
        return buffer